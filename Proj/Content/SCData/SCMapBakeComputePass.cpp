...
namespace engine
{
	...
	bool SCMapBakeComputePass::CreateTileSetGPUData(const std::vector<TileSet>& tileset_data)
	{
		tileset_gpu_data_.resize((size_t)TileSetType::kEND);
		std::vector<uint16> megatile_data;
		bool result = false;

		//index tileset
		for (size_t tileset_i = 0; tileset_i < tileset_gpu_data_.size(); ++tileset_i)
		{
			const TileSet& tileset = tileset_data[tileset_i];
			TileSetGPUData& tileset_gpu = tileset_gpu_data_[tileset_i];
...
#pragma region CV5 - Megatile
			...
			if (!result) { tileset_gpu_data_.clear(); return false; }
...
#pragma endregion WPE - Color Palette
	...
	s_ptr<Texture2D> SCMapBakeComputePass::BakeMapTexture(const MapInfo& map_info)
	{
		if (map_info.megatile_width == 0 || map_info.megatile_height == 0)
		{
			ERROR_MESSAGE("Invalid map size. Width and height must be greater than 0.");
			return nullptr;
		}
		if (map_info.mtxm.size() < (map_info.megatile_width * map_info.megatile_height))
		{
			ERROR_MESSAGE("MTXM data is invalid.");
			return nullptr;
		}
		if ((size_t)map_info.terrain_type >= tileset_gpu_data_.size())
		{
			ERROR_MESSAGE("Invalid terrain type or TileSet GPU data is not created.");
			return nullptr;
		}

		const auto& tileset_gpu = tileset_gpu_data_[(size_t)map_info.terrain_type];
		if (tileset_gpu.CV5_megatiles == nullptr ||
		...
		desc.Width = map_info.megatile_width * (UINT)megatile_size;
		desc.Height = map_info.megatile_height * (UINT)megatile_size;
		...
		std::span<const MTXM> mtxm_span = map_info.mtxm;
